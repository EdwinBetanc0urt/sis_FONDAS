...
function UltimoCodigoMunicipio() {
	$objMunicipio = new Municipio(); //instancia la clase
	$arrCodigo = $objMunicipio->UltimoCodigo(); //obtiene el arreglo con el codigo
...
function borrar() {
	...
	$objMunicipio = new Municipio();
	$objMunicipio->setFormulario($_POST);
	if ($objMunicipio->Eliminar()) //si el fmInsertar es verdadero, realiza las sentencias
...
function ListaMunicipio() {
	...
	$objeto = new Municipio; //instancia la clase
		...
		while ($arrRegistro = $objeto->getConsultaAsociativo($rstRecordSet)) {
			?>
						<tr onclick='fjSeleccionarRegistro(this);' data-toggle='tooltip' data-placement='top' title='Doble clic para detallar los datos y realizar alguna operación'
							...
							<td> <?= html_entity_decode(strtolower( $arrRegistro[$objeto->atrNombre]), ENT_COMPAT | ENT_HTML5, 'UTF-8') ; ?> </td>
