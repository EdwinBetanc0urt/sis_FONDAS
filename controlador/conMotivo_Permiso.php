...
function UltimoCodigoMotivo_Permiso() {
	$objMotivo_Permiso = new Motivo_Permiso(); //instancia la clase
	$arrCodigo = $objMotivo_Permiso->UltimoCodigo(); //obtiene el arreglo con el codigo
...
function borrar() {
	...
	$objMotivo_Permiso = new Motivo_Permiso();
	$objMotivo_Permiso->setFormulario($_POST);
	if ( $objMotivo_Permiso->Eliminar() ) //si el fmInsertar es verdadero, realiza las sentencias
					...
					<thead>
						<tr class='info'>
                            ...
                            <th datos_orden_metodo="asc" datos_orden="<?= $objeto->atrCantidad_Dias; ?>" onclick='fjMostrarLista( "<?= $gsClase; ?>" , "<?= $vpPaginaActual; ?>" , "<?= $objeto->atrCantidad_Dias; ?>" )' >
								Duracion Dias <span class='glyphicon glyphicon-sort'></span>
							</th>
                            <th datos_orden_metodo="asc" datos_orden="<?= $objeto->atrCantidad_Tiempo; ?>" onclick='fjMostrarLista( "<?= $gsClase; ?>" , "<?= $vpPaginaActual; ?>" , "<?= $objeto->atrCantidad_Tiempo; ?>" )' >
								Duracion Minutos <span class='glyphicon glyphicon-sort'></span>
							</th>
		...
		while ($arrRegistro = $objeto->getConsultaAsociativo( $rstRecordSet ) ) {
			?>
						<tr onclick='fjSeleccionarRegistro( this );' data-toggle='tooltip' data-placement='top' title='Doble clic para detallar los datos y realizar alguna operación'
                            ...
                            |<?= ucwords( $arrRegistro[ "cantidad_dias" ] ); ?>
							|<?= ucwords( $arrRegistro[ "cantidad_tiempo" ] ); ?>' >
								<!-- FINAL DE LA APERTURA DEL TR DE LA FILA -->

							<td> <?= $arrRegistro[ $objeto->atrId ]; ?> </td>
							<td> <?= ucwords( $arrRegistro[ $objeto->atrNombre ] ); ?> </td>
                            <td> <?= ucwords( $arrRegistro[ $objeto->atrCantidad_Dias ] ); ?> </td>
                            <td> <?= ucwords( $arrRegistro[ $objeto->atrCantidad_Tiempo ] ); ?> </td>
