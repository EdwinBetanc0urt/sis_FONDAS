...
function UltimoCodigoTipo_Ausencia() {
	$objTipo_Ausencia = new Tipo_Ausencia(); //instancia la clase
	$arrCodigo = $objTipo_Ausencia->UltimoCodigo(); //obtiene el arreglo con el codigo
...
function registrar() {
	global $gsClase;
	$objTipo_Ausencia = new Tipo_Ausencia();
	$objTipo_Ausencia->setFormulario( $_POST );

	$arreglo = $objTipo_Ausencia->consultar(); //realiza una consulta
	//si existe un registro
	if ( $arreglo ) {
		//envía a la vista, con mensaje de la consulta
		header( "Location: ../?form={$gsClase}" .
			"&msjAlerta=duplicado&getOpcion=" . $_POST["operacion"] .
			"&getId=" . $arreglo[ $objTipo_Ausencia->atrId ] .
			"&getNombre=" . $arreglo[ $objTipo_Ausencia->atrNombre ] .
			"&getEstatus=" . $arreglo[ $objTipo_Ausencia->atrEstatus ] );  
	} //cierre del condicional si el RecordSet es verdadero
	else {
		if ( $objTipo_Ausencia->Incluir() ) //si el fmInsertar es verdadero, realiza las sentencias
...
function cambiar() {
	...
	$objTipo_Ausencia = new Tipo_Ausencia();
	$objTipo_Ausencia->setFormulario($_POST);
	//var_dump( $objTipo_Ausencia->Modificar() );/*
	if ( $objTipo_Ausencia->Modificar() ) //si el fmInsertar es verdadero, realiza las sentencias
...
function borrar() {
	...
	$objTipo_Ausencia = new Tipo_Ausencia();
	$objTipo_Ausencia->setFormulario($_POST);
	if ( $objTipo_Ausencia->Eliminar() ) //si el fmInsertar es verdadero, realiza las sentencias
				...
				<table border='0' valign='center' class='table table-striped text-center table-hover' id="tabLista<?= $gsClase; ?>">
					<thead>
						<tr class='info'>
							...
							<th datos_orden_metodo="asc" datos_orden="<?= $objeto->atrNombre; ?>" onclick='fjMostrarLista( "<?= $gsClase; ?>" , "<?= $vpPaginaActual; ?>" , "<?= $objeto->atrDescripcion; ?>" )' >
