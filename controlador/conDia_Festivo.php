...
function UltimoCodigoDia_Festivo() {
	$objDia_Festivo = new Dia_Festivo(); //instancia la clase
	$arrCodigo = $objDia_Festivo->UltimoCodigo(); //obtiene el arreglo con el codigo
...
function cambiar() {
	global $gsClase;
	$objDia_Festivo = new Dia_Festivo();
	$objDia_Festivo->setFormulario($_POST);
	//var_dump($objDia_Festivo->Modificar());/*
	if ($objDia_Festivo->Modificar()) //si el fmInsertar es verdadero, realiza las sentencias
...
function borrar() {
	...
	$objDia_Festivo = new Dia_Festivo();
	$objDia_Festivo->setFormulario($_POST);
	if ($objDia_Festivo->Eliminar()) //si el fmInsertar es verdadero, realiza las sentencias
		...
		while ($arrRegistro = $objeto->getConsultaAsociativo($rstRecordSet)) {
			?>
						<tr onclick='fjSeleccionarRegistro(this);' data-toggle='tooltip' data-placement='top' title='Doble clic para detallar los datos y realizar alguna operación'
							...
							<td> <?=  $objeto->faFechaFormato($arrRegistro["fecha"], "amd", "dM"); ?> </td>
