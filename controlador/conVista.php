...
function UltimoCodigoVista() {
	$objVista = new Vista(); //instancia la clase
	$arrCodigo = $objVista->UltimoCodigo(); //obtiene el arreglo con el codigo
...
function registrar() {
	...
	if ($arreglo) {
		...
		header("Location: ../?form={$gsClase}" .
			...
			"&getPosicion=" . $arreglo["orden"] .
			"&getUrl=" . $arreglo["url"] .
			"&getModulo=" . $arreglo["idmodulo"] .
...
function cambiar() {
	global $gsClase;
	$objVista = new Vista();
	$objVista->setFormulario($_POST);
	//var_dump($objVista->Modificar());/*
	if ($objVista->Modificar()) //si el fmInsertar es verdadero, realiza las sentencias
...
function borrar() {
	...
	$objVista = new Vista();
	$objVista->setFormulario($_POST);
	if ($objVista->Eliminar()) //si el fmInsertar es verdadero, realiza las sentencias
...
function ListaVista() {
	...
	$objeto = new Vista; //instancia la clase
