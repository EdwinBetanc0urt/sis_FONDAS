...
function UltimoCodigoParroquia() {
	$objParroquia = new Parroquia(); //instancia la clase
	$arrCodigo = $objParroquia->UltimoCodigo(); //obtiene el arreglo con el codigo
...
function registrar() {
	...
	if ( $arreglo ) {
		...
		header( "Location: ../?form={$gsClase}" .
			...
			"&getEstado=" . $arreglo[ "idestado" ] .
			"&getMunicipio=" . $arreglo[ "idmunicipio" ] .
...
function cambiar() {
	...
	$objParroquia->setFormulario($_POST);
	//var_dump( $objParroquia->Modificar() );/*
	if ( $objParroquia->Modificar() ) //si el fmInsertar es verdadero, realiza las sentencias
...
function borrar() {
	...
	$objParroquia = new Parroquia();
	$objParroquia->setFormulario($_POST);
	if ( $objParroquia->Eliminar() ) //si el fmInsertar es verdadero, realiza las sentencias
...
function Combo() {
    ...
    if ( isset( $_POST["hidCodPadre"] ) )
        $pvCodigoPadre =  htmlentities( trim ( addslashes( strtolower( $_POST["hidCodPadre"] ) ) ) );
    else
        $pvCodigoPadre = "";
    
    $lsSeleccionado = "";
    $objeto = new Parroquia();
    $rstRecordSet = $objeto->Listar( $pvCodigoPadre );
				...
				<table border='0' valign='center' class='table table-striped text-center table-hover' id="tabLista<?= $gsClase; ?>">
					<thead>
						<tr class='info'>
							...
							<th datos_orden_metodo="asc" datos_orden="<?= $objeto->atrEstatus; ?>" onclick='fjMostrarLista( "<?= $gsClase; ?>" , "<?= $vpPaginaActual; ?>" , "municipio")' >
								Municipio  <span class='glyphicon glyphicon-sort'></span>
							</th>
							<th datos_orden_metodo="asc" datos_orden="<?= $objeto->atrEstatus; ?>" onclick='fjMostrarLista( "<?= $gsClase; ?>" , "<?= $vpPaginaActual; ?>" , "estado"' >
		...
		while ($arrRegistro = $objeto->getConsultaAsociativo( $rstRecordSet ) ) {
			?>
						<tr onclick='fjSeleccionarRegistro( this );' data-toggle='tooltip' data-placement='top' title='Doble clic para detallar los datos y realizar alguna operación'
							...
							|<?= $arrRegistro[ "idmunicipio" ] ; ?>
							|<?= $arrRegistro[ "municipio" ] ; ?>
							|<?= $arrRegistro[ "idestado" ] ; ?>
							|<?= ucwords( $arrRegistro[ "estado" ] ); ?>' >
								<!-- FINAL DE LA APERTURA DEL TR DE LA FILA -->

							<td> <?= $arrRegistro[ $objeto->atrId ]; ?> </td>
							<td> <?= $arrRegistro[ $objeto->atrNombre ]; ?> </td>
							<td> <?= $arrRegistro["municipio"]; ?> </td>
							<td> <?= $arrRegistro["estado"]; ?> </td>
