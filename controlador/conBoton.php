...
function UltimoCodigoBoton() {
	$objBoton = new Boton(); //instancia la clase
...
function registrar() {
	...
	$arreglo = $objBoton->consultar(); //realiza una consulta
	//si existe un registro
	if ($arreglo) {
		//envía a la vista, con mensaje de la consulta
		header("Location: ../?form={$gsClase}" .
			"&msjAlerta=duplicado&getOpcion=" . $_POST["operacion"] .
			"&getId=" . $arreglo[$objBoton->atrId] .
			"&getNombre=" . $arreglo[$objBoton->atrNombre] .
			"&getEstatus=" . $arreglo[$objBoton->atrEstatus]);  
	} //cierre del condicional si el RecordSet es verdadero
	else {
		if ($objBoton->Incluir()) //si el fmInsertar es verdadero, realiza las sentencias
...
function cambiar() {
	...
	$objBoton = new Boton();
	$objBoton->setFormulario($_POST);
	//var_dump($objBoton->Modificar());/*
	if ($objBoton->Modificar()) //si el fmInsertar es verdadero, realiza las sentencias
...
function borrar() {
	...
	$objBoton = new Boton();
	$objBoton->setFormulario($_POST);
	if ($objBoton->Eliminar()) //si el fmInsertar es verdadero, realiza las sentencias
...
function ListaBoton() {
	...
	$objeto = new Boton; //instancia la clase
