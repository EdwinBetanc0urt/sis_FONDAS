...
class Dia_Semana extends clsConexion {
	...
	function __construct() {
		...
		$this->atrTabla = "tdia_semana";
		$this->atrId = "iddia_semana";
	...
	function Incluir() {
		$sql = "
			INSERT INTO {$this->atrTabla} ( {$this->atrNombre}, nro_dia ) 
			VALUES ( 
				'{$this->atrFormulario["ctxNombre"]}',
				'{$this->atrFormulario["numDia"]}'
			); ";
	...
	function Modificar() {
		$sql = "
			...
			SET 
				...
				nro_dia = '{$this->atrFormulario["numDia"]}' 
			WHERE 
