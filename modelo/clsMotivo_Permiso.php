...
class Motivo_Permiso extends clsConexion {
	...
	function __construct() {
		...
		$this->atrId = "idmotivo_permiso";
        $this->atrNombre = "nombre";
        $this->atrCantidad_Dias= "cantidad_dias";
        $this->atrCantidad_Tiempo = "cantidad_tiempo";
	...
	function Modificar() {
		$sql = "
			...
			SET 
                ...
                cantidad_tiempo = '{$this->atrFormulario["ctxCantidad_Tiempo"]}'                  
			WHERE 
	...
	function fmListarIndex($psBuscar) {		
		$sql = "
			...
			WHERE
                ...
                nombre LIKE '%{$psBuscar}%' OR
                cantidad_dias LIKE '%{$psBuscar}%' OR 
				cantidad_tiempo LIKE '%{$psBuscar}%') "; //selecciona todo de la tabla
